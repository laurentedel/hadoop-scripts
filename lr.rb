def list_regions(table_name, region_server_name = "")
  ...
  printf("%-60s | %-32s | %-15s | %-15s | %-15s | %-15s | %-15s", "SERVER_NAME", "ENCODED_REGION_NAME", "START_KEY", "END_KEY", "SIZE(MB)", "OCCUPANCY(%)", "REQ");
  printf("\n")
  for result in results
    # delete control characters and limit to 30 chars for StartKey and EndKey
    sk = result["startkey"].gsub!(/[^0-9A-Za-z]/, '')
    sk = "[EMPTY]" if sk.nil?
    startkey = sk.length > 29 ? sk.slice(1..24) + "[...]" : sk
    ek = result["endkey"].gsub!(/[^0-9A-Za-z]/, '')
    ek = "[EMPTY]" if ek.nil?
    endkey = ek.length > 29 ? ek.slice(1..24) + "[...]" : ek
    encoded_region_name = result["name"].split(",")[2].split(".")[1]
    printf("%-60s | %-32s | %-15s | %-15s | %-15s | %-15s | %-15s", result["server"], encoded_region_name, startkey, endkey, result["size"], result["occupancy"], result["requests"]);
    printf("\n")
  end
